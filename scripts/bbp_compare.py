...
def run_full(body_fitted: bool = False) -> None:
    ...
    print(f"[NEURON] {len(section_info)} sections, "
          ...
          f"V_m peak {nrun.rec_v_mV['soma(0.5)'].max():.1f} mV in {_wall(t_neuron)}")
    ...
    # Soma center: anchor TetGen's interior-point at a known-cell
    # location, since fem_neuron's default heuristic fails on
    # complex morphologies (Hay 2011 L5 PC's 1.4 mm apical dendrite
    # makes the mesh COM land outside the cell).
    soma_anchor = None
    for info in section_info:
        if "soma" in info["name"].lower():
            soma_anchor = info["points_um"].mean(axis=0)
            break
    ...
    if body_fitted:
        ...
        print("[FEM]    body-fitted: AMS + TetGen ...")
        t0 = time.time()
        ...
        section_tags = [TAG_MEMBRANE_BASE + i for i in range(len(section_info))]
        print(f"[FEM]    branched: {len(sections)} sections ...")
        t0 = time.time()
...
def plot_overlay() -> None:
    ...
    near_i = int(order[0]); mid_i = int(order[len(order) // 2])
    far_i = int(order[-1])
    ...
    ax_vm.set_ylabel("V_m (mV)"); ax_vm.set_xlabel("t (ms)")
    ax_vm.legend(fontsize=9); ax_vm.grid(alpha=0.3)
    ...
    ax_radial.plot(r_sorted[finite_mask],
                   v_e_fem[snap_idx, order][finite_mask], "s--",
    ...
    ax_radial.legend(fontsize=9); ax_radial.grid(alpha=0.3, which="both")

    plt.suptitle("Hay 2011 L5 PC — FEM vs LSA", y=0.995, fontsize=12)
    ...
    for idx in order:
        ...
        if np.isnan(f):
            print(f"  {radii[idx]:8.1f}  {l:10.3f}  {'nan':>10}  {'nan':>8}")
            continue
